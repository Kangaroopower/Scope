/* GUI */
	/* GUI operations- open/close (Actual dialog is stored elsewhere) */
	Scope.Sequencer('lib').onload(function () {
		$('#sc-start').click(function () {
			if (document.querySelector('#sc-ui')) {
				...
				Scope.Sequencer('GUI').loaded();
			}				
		});
	}());

	/* Layout Shadow's GUI */
	Scope.Sequencer('GUI').onload(function () {
		...
		sctxtarea.css(commonCSS).css({position: 'relative', zIndex: '1',
