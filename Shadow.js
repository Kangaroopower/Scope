...
(function () {
	...
	$.sub('synch', function () {
		...
		var ref = 0;
		matches = [];
		while(sctxtarea.val().indexOf(scfind, ref) !== -1) {
			if (!$('input#sc-cs').is(':checked')) matches.push(sctxtarea.val().toLowerCase().indexOf(scfind));
			else matches.push(sctxtarea.val().toLowerCase().indexOf(scfind));
	...
	Scope.dir = function  (next) {
		if (next === true) for (var n = 0; n < matches.length; n++) if (sel.end < matches[n] + matches[n].length) highlight(n);
		else for (var p = matches.length-1; p >= 0; p--) if (sel.start > matches[p]) highlight(p);
	};
})();
