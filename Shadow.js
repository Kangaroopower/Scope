...
Scope.Shadow.init = function () {
	Scope.textarea.after('<div id="sc-shadow" style="left: 0px; top: 0px; border: 0px none; display: block; outline: none medium; margin: 0px; padding: 0px; resize: none; position: absolute; z-index: 0; font-family: Consolas, Eupheima UCAS, Ayuthaya, Menlo, monospace; font-size: 14px; line-height: normal; white-space: pre-wrap; background-color: transparent; color: transparent; overflow: auto; height: 529px; "></div>');
	var commonCSS = {
		width: '100%', left: 0, top: 0, border: '0 none', display: 'block',
		outline: 'medium none', margin: 0, padding: 0, resize: 'none'
	};
	Scope.textarea.css(commonCSS).css({
		position: 'relative', zIndex: '1',
		backgroundColor: 'transparent'
	});
	Scope.textarea.scroll(function () {
		$('#sc-shadow').scrollTop(Scope.textarea.scrollTop());
	});
	Scope.textarea.focus();
	Scope.textarea.keyup (Scope.Shadow.synch);
	Scope.textarea.click (Scope.Shadow.synch);
	Scope.Shadow.synch();
};
...
Scope.Shadow.synch = function() {
	...
	$('#sc-shadow').css('height', Scope.textarea.height()); 
	$('#sc-shadow').css('width', Scope.textarea.width());
	$('#sc-shadow').scrollTop(Scope.textarea.scrollTop());
};
...
Scope.Shadow.highlight = function(high) {
	...
	Scope.textarea.setSelection(Scope.Shadow.matches[high].index, Scope.Shadow.matches[high].index + Scope.Shadow.matches[high].found.length);
...
Scope.Shadow.next = function  () {
	...
	Scope.textarea.focus();
	var n = 0;
	var sel = Scope.textarea.getSelection();
